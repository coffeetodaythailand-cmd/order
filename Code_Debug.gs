 ...
 * แก้ไขล่าสุด: 12 Mar 2026 (ปรับปรุง Footer อีเมลตามคำสั่งบอส + Clean Code)
...
function logSystemError(functionName, errorMessage, payloadData) {
  try {
    ...
    // ---------------------------------------------------------
    // 🚀 ย้ายระบบจัดการเมนูมาใช้ POST เพื่อความเสถียรขั้นสุด!
    // ---------------------------------------------------------
...
function submitOrderBackupOnly(f, externalOrderId) {
  ...
  try {
    ...
    if (externalOrderId) {
      ...
      // แอบไปอัปเดต C3 ใน MasterData ให้ทันสมัยด้วย เผื่อ Admin มาสั่งเองทีหลัง
