...
function getOrdersByStatus(statusArray, searchQ) {
  ...
  try {
    ...
    var recentData = dataRange.getDisplayValues();
    ...
    for (var i = 0; i < recentData.length; i++) {
      ...
      if (isMatch) {
        ...
        var rawDateObj = rawRow[0];
        var finalDateStr = row[0] ? row[0].toString().trim() : "ไม่ระบุเวลา";
        var finalTimeStr = "-";
        
        if (Object.prototype.toString.call(rawDateObj) === '[object Date]' && !isNaN(rawDateObj.getTime())) {
          finalDateStr = Utilities.formatDate(rawDateObj, "GMT+7", "dd/MM/yyyy HH:mm");
          finalTimeStr = Utilities.formatDate(rawDateObj, "GMT+7", "HH:mm");
        }
